...
const gachaTable = [
    ...
    { name: "흙 쿠키", odds: 10000000, grade: "common" }, // 나머지 모두
];
...
function startBattle() {
    if (equippedBattleCookie) {
        alert(`'${equippedBattleCookie}'(으)로 맵에 입장합니다. (개발 중)\n\n(컨셉: 맵의 적들을 ${equippedBattleCookie}(으)로 발사하여 쓰러트리고 상자를 파밍합니다.)`);
    } else {
...
function openSidebar(tabName) {
    mainSidebar.classList.add('is-open');
...
function switchTab(tabName) {
    ...
    shopTabContent.classList.add('hidden');
    rankingTabContent.classList.add('hidden');
    ...
    if (tabName === 'inventory') {
        ...
        shopTabContent.classList.remove('hidden');
        shopTabButton.classList.add('active');
    } else if (tabName === 'ranking') {
        rankingTabContent.classList.remove('hidden');
...
function switchCentralPanel(mode) {
    strengthenPanel.classList.add('hidden');
    ...
    if (mode === 'strengthen') {
        strengthenPanel.classList.remove('hidden');
...
function updateDisplay() {
    ...
    if (level > 0) {
        ...
        protectionToggleLabel.classList.remove('hidden');
        ...
        protectionToggleLabel.classList.add('hidden');
        protectionToggleCheckbox.checked = false;
        protectionToggleCheckbox.disabled = true;
    }
}
